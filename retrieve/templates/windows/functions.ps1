...
function Add-ToConfig {
    ...
    if($configContent -Match [System.Text.RegularExpressions.Regex]::Escape($Line)) {
        Write-Host "$Line already present in configuration."
        return
    }
    Write-Host "* Adding `"$Line`" to [$Block]"
...
function Push-InterpretersToConfig {
    if (-Not([System.Version]$targetVersion -ge [System.Version]"0.5.12")) {
        Write-Host "* RPort version $targetVersion does not support Interpreter Aliases"
        return
    }
    Write-Host "* Looking for script interpreters."
    $interpreters = Find-Interpreters
    Write-Host "* $($interpreters.count) script interpreters found."
...
function Install-Tacoupdates {
    ...
    if ((Out-String -InputObject (& 'C:\Program Files\tacoscript\bin\tacoscript.exe' --version)) -match "Version: (.*)") {
        ...
        If ((Get-Item tacoupdate.zip).length -eq 0) {
            Write-Host "* No Tacoscript update needed. You are on the latest $tacoVersion version."
        ...
        Write-Host "* Tacoscript updated to $( (& "$( $dest )\bin\tacoscript.exe" --version) -match "Version" )"
...
function Install-Tacoscript {
    ...
    if (Test-Path -Path $tacoBin) {
        Write-Host "* Tacoscript already installed to $( $tacoBin )"
        Install-Tacoupdates
        return
    }
    $Temp = [System.Environment]::GetEnvironmentVariable('TEMP', 'Machine')
    ...
    Write-Host "* Tacoscript dowloaded to $( $file )"
    ...
    Write-Host "* Tacoscript installed to '$( $tacoDir )' $( (tacoscript.exe --version) -match "Version" )"
    rm $file -force
...
'
    Write-Host "* Tacoscript uninstaller created in $( $tacoDir )\uninstall.bat."
}
...
function Add-Netcard {
    ...
    if($configContent -match "[^#]\s*$Type = \[") {
        Write-Host "* Monitoring for $Type '$InterfaceAlias' already activated. Skipping."
        return
    }
    if($configContent -match "#+\s*$($Type) =") {
    ...
    Write-Host "* Monitoring for $Type '$InterfaceAlias' activated."
    $Utf8NoBomEncoding = New-Object System.Text.UTF8Encoding $False
    [IO.File]::WriteAllLines($configFile, $configContent, $Utf8NoBomEncoding)
}
...
function Enable-Network-Monitoring {
    if (-Not([System.Version]$targetVersion -ge [System.Version]"0.5.8")) {
        Write-Host "* RPort version $targetVersion does not support Lan/Wan Monitoring"
        return
    }
    if ((get-content $configFile) -match "^\s*net_[lw]an") {
        Write-Host "* Network Monitoring already enabled"
        return
    }
    ...
    if (-Not$netLan -and -Not$netWan) {
        Write-Host "* No Lan cards detected. Check manually with 'Get-NetAdapter'"
        return
    }
    if($netLan) {
